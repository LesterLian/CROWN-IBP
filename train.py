...
from ada import get_weighted_ce
...
def Train(model, t, loader, eps_scheduler, max_eps, norm, logger, verbose, train, opt, method, **kwargs):
    ...
    if batch_multiplier > 1 and train:
        logger.log('Warning: Large batch training. The equivalent batch size is {} * {} = {}.'.format(batch_multiplier,
                                                                                                      loader.batch_size,
                                                                                                      batch_size))
    ...
    if end_eps < np.finfo(np.float32).tiny:
        ...
        method = "natural"
    # Fixme remove distribution, idx if possible
    for i, data in enumerate(loader):
        if train:
            data, labels, distribution, idx = data
            ce_loss = get_weighted_ce(distribution)
        else:
        ...
        # Fixme
        regular_ce = ce_loss()(output, labels)
        ...
        if verbose or method != "natural":
            if kwargs["bound_type"] == "convex-adv":
                ...
                ub, lb, relu_activity, unstable, dead, alive = model(norm=norm, x_U=data_ub, x_L=data_lb, eps=eps, C=c,
                ...
                if beta < 1e-5:
                    ...
                    if kwargs["runnerup_only"]:
                        ...
                        _, _, clb, bias = model(norm=norm, x_U=data_ub, x_L=data_lb, eps=eps, C=c,
...
def main(args):
    ...
    for model, model_id, model_config in zip(models, model_names, config["models"]):
        ...
        for t in range(epochs):
            ...
            logger.log(
                "Epoch {}, learning rate {}, epsilon {:.6g} - {:.6g}".format(t, lr_scheduler.get_lr(), epoch_start_eps,
                                                                             epoch_end_eps))
            # with torch.autograd.detect_anomaly():
            start_time = time.time()
            Train(model, t, train_data, epsilon_scheduler, max_eps, norm, logger, verbose, True, opt, method,
                  **method_param)
            if lr_decay_step:
            ...
            with torch.no_grad():
                # evaluate
                err, clean_err = Train(model, t, test_data,
                                       EpsilonScheduler("linear", 0, 0, epoch_end_eps, epoch_end_eps, 1), max_eps, norm,
                                       logger, verbose, False, None, method, **method_param)
